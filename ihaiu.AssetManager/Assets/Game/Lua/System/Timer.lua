...
function Timer.New(func, duration, loop, group, scale)
	local timer = {}
	...
	if group then
		if not timerTable[group] then
			timerTable[group] = {}
		end
		table.insert(timerTable[group], timer)
	end
	return timer
end

function Timer.StopGroup(group)
	if timerTable[group] then
		for k,v in pairs(timerTable[group]) do
			v:Stop()
		end
	end
end

function Timer:Start(runFirst)
	self.running = true
...
function Timer:Update()
	...
	if self.time <= 0 and Time.frameCount > self.count then
		local ret = self.func(self.duration - self.time)
		if ret then
			self:Stop()
		end
		...
		if self.loop > 0 then
			...
			local s = self.time < 0 and -1 or 1
			self.time = s * (math.abs(self.time) % self.duration) + self.duration
		end
