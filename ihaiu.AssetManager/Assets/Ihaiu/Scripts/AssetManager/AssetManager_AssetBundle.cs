...
namespace com.ihaiu
{
    public partial class AssetManager 
    {
        ...
        public IEnumerator InitManifest()
        {
            ...
            #endif
            {
                yield return StartCoroutine(manifestAssetBundleManager.LoadManifest(AssetManagerSetting.FileURL.ManifestURL));
            }
        }
        ...
        void CreateLoadedAssetBundleCache(string assetBundleName, string assetName, System.Type type, UnityEngine.Object obj)
        {
            ...
            if (loaded != null)
            {
                if (loaded.obj != obj && loaded.obj != null)
                {
                    loaded.UnloadObj();
                }
        ...
        public void ForceClearAllAssetBundleCache()
        {
            LoadedAssetBundleCache loaded;
            for(int i = LoadedAssetBundleCacheList.Count - 1; i >= 0; i --)
            {
        ...
        IEnumerator OnLoadAssetAsync(string filename, string assetBundleName, string assetName, System.Type type, Action<string, object, object[]> callback, params object[] callbackArgs)
        {
            ...
            yield return CheckImmediatelyUnloadAssetBundle(assetBundleName);
        }

        protected IEnumerator CheckImmediatelyUnloadAssetBundle(string assetBundleName)
        {
            if (assetBundleName.StartsWith("module/") || assetBundleName.StartsWith("effect3d/"))
            {
               // yield return new WaitForSeconds(1);
               // manifestAssetBundleManager.UnloadAssetBundleInternal(assetBundleName);
            }
            else if(assetBundleName.StartsWith("image/") || assetBundleName.StartsWith("unit_prefab/") || assetBundleName.StartsWith("map/"))
            {
                yield return new WaitForSeconds(1);
                manifestAssetBundleManager.UnloadAssetBundle(assetBundleName);
            }
